...
#include "ReceiverCode/simple_sockets.hpp"
#include "ReceiverCode/sensor_aggregator_protocol.hpp"
...
public:
  ...
  void connectToReader(SampleData* sd) {    
    //Set up the USB
    usb_init(); 
    usb_find_busses(); 
    usb_find_devices(); 
  ...
  void readTag(SampleData* sd, int pip_id){
    if (pip_devs.size() > 0) {
        ...
        //If the length of the message is equal to or greater than PACKET_LEN then this is a data packet.
          ...
          if ((pkt->rssi != (int) 0) and (pkt->status != 0)) {
            ...
            if (not parity_failed) {
              ...
              // Now passing in sample data class
              ...
              unsigned int netID = ((unsigned int)data[9] * 65536)  + ((unsigned int)data[10] * 256) + ((unsigned int)data[11] );
              ...
              pip_light = (unsigned int)sd->sense_data[1];
              pip_temp =  (unsigned int)sd->sense_data[2]*256+(unsigned int)sd->sense_data[3];
            }
          }           
        }
